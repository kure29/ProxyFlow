...
| Shadowsocks | The tested client recognized Shadowsocks `aes-256-gcm`; other cipher/plugin combinations remain unproven. | Method, password, opaque plugin metadata. | Allow only the audited method and simple-obfs subset; all other plugin/options fail closed. | Conditional | `SHADOWROCKET_SHADOWSOCKS_CIPHER_UNPROVEN`, `SHADOWROCKET_SHADOWSOCKS_PLUGIN_UNPROVEN` | Shadowrocket 2.2.65 build 2615 core traffic acceptance |
...
| Select/manual | The tested client recognized and selected both core members. | Select strategy with ordered candidates. | Serialize as `select` for the tested emitted subset; no broader selection claim. | Conditional | `SHADOWROCKET_STRATEGY_UNPROVEN` | Shadowrocket 2.2.65 build 2615 core acceptance |
| URL test / auto-select | The tested client used the emitted health URL and selected automatically. | URL, interval, tolerance. | Serialize explicit health fields; fallback tolerance remains blocked. | Conditional | `SHADOWROCKET_STRATEGY_TEST_URL_INVALID`, `SHADOWROCKET_FALLBACK_TOLERANCE_UNPROVEN` | Shadowrocket 2.2.65 build 2615 URL-test acceptance |
| Fallback | The tested client timed out an unavailable member and selected the remaining healthy member. | Ordered fallback candidates and health check. | Conditional; preserve order, reject unproven tolerance semantics. | Conditional | `SHADOWROCKET_STRATEGY_UNPROVEN` | Shadowrocket 2.2.65 build 2615 fallback acceptance |
| Load balance | Round-robin behavior was observed across connections; exact distribution is unclaimed. | Round-robin/consistent-hash mode. | Serialize explicit round-robin only; consistent-hash remains blocked. | Conditional | `SHADOWROCKET_STRATEGY_UNPROVEN` | Shadowrocket 2.2.65 build 2615 load-balance acceptance |
...
| DNS system / UDP | System DNS import/behavior and IPv4 UDP resolver recognition were observed in the tested client. | DnsIR system/UDP default resolvers. | Current adapter limits output to `dns-server` system/IPv4 literals; no specific system address or other resolver role is inferred. | Conditional | `SHADOWROCKET_DNS_*` | Shadowrocket 2.2.65 build 2615 DNS acceptance |
...
| Remote proxy sources | Native subscription format/refresh contract remains unproven; a materialized local subscription was accepted. | Remote source URL, request profile, export mode, snapshot. | Materialized snapshots may be compiled by the normal IR path; native remote export fails. | Accepted limitation | `SHADOWROCKET_REMOTE_PROXY_SOURCE_UNPROVEN` | Shadowrocket 2.2.65 build 2615 materialized-subscription acceptance |
