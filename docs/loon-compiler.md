...
| HTTP, username/password | Positional username and a simple quoted password are documented. | Optional `username` and `password` are exact fields. | Emit both credentials together only for the proven simple token subset; reject a half-pair and any delimiter/escape-sensitive value until parser evidence proves a broader password grammar. | Supported | `LOON_PROXY_AUTH_UNSUPPORTED`, `LOON_SERIALIZER_UNSAFE_VALUE` | [node.md#L61-L64](https://github.com/Loon0x00/LoonManual/blob/4311d0030fe3065d4664b403a32010f083b99273/docs/cn/node.md#L61-L64) |
...
| VMess, TCP/WS/HTTP | Loon documents UUID, security, `transport=tcp`, `ws`, or `http`, `alterId`, path/host, TLS, SNI, and certificate skipping. The pinned examples use `aes-128-gcm`; `alterId=0` is described as enabling AEAD. | IR carries UUID/security and optional `alterId`, TLS, and transports, but an omitted `alterId` does not prove the AEAD intent. | Conditional support requires the directly evidenced `security=aes-128-gcm`, explicit `alterId` (including explicit zero), and only documented TCP/WS/HTTP fields. Never infer `auto`, `none`, or another security value, and never auto-fill `alterId=0`. | Conditional | `LOON_VMESS_VARIANT_UNSUPPORTED`, `LOON_PROXY_CIPHER_UNSUPPORTED` | [node.md#L72-L94](https://github.com/Loon0x00/LoonManual/blob/4311d0030fe3065d4664b403a32010f083b99273/docs/cn/node.md#L72-L94) |
...
The official examples are comma-delimited and quote some simple passwords, but
the audited manual does not specify a complete escape grammar for arbitrary
names or values. Foundation therefore does not invent CSV-like quoting or
backslash escapes. The serializer accepts only a directly representable,
non-empty printable ASCII token with no delimiter, quote, backslash, comment
marker, or ambiguous outer whitespace. A value outside that grammar fails
closed with `LOON_SERIALIZER_UNSAFE_VALUE`; no partial profile is emitted.

- comma and equals are unsafe until a pinned parser/client fixture proves their
  representation;
- quote and backslash are unsafe because no escape semantics are pinned;
- CR, LF, NUL, other control characters, and line separators are always unsafe;
- Unicode is not guaranteed to round-trip without first-party parser/client
  evidence and is therefore blocked in this foundation;
- protocol serializers all use the same simple-token policy and never quote or
  escape individual values differently.

Serializer fixtures cover a proven simple value plus comma, equals, quote,
backslash, CR/LF/NUL, and Unicode rejection. The official quoted-password
examples justify the field's presence, not arbitrary embedded-character
escaping.
