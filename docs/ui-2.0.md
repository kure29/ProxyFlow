...
3. compatibility for the active supported target;
4. Preview and Export actions.

Only Mihomo is actionable. sing-box official export is paused, while Surge,
Loon, Quantumult X, Shadowrocket, and Stash are not presented as production
exports. Target cards show readiness, blocker codes, and node count without
depending on a narrow panel. Mihomo exposes its existing Local Proxy / Desktop
TUN profile and related network, DNS, and Advanced options.

The normal product path compiles and validates Mihomo only, so a hidden
sing-box limitation cannot reduce Mihomo compatibility or disable its download.
Historical sing-box Projects retain their data, show a paused status, and can
switch to Mihomo. Download filenames and MIME types are generated by a tested
target export helper.
...
- Mihomo is the only production output target. sing-box compiler infrastructure
  remains registered but paused in the product surface.
