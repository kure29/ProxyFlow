...
const issueCopy: Record<string, Record<Locale, string>> = {
  ...
  SUBSCRIPTION_NETWORK_ERROR: { 'en-US': 'The current runtime could not reach the subscription server.', 'zh-CN': '当前运行环境无法连接订阅服务器。' },
  ...
  REMOTE_SOURCE_NATIVE: { 'en-US': 'This target will load the remote subscription directly.', 'zh-CN': '该目标将直接加载远程订阅。' },
  REMOTE_SOURCE_MATERIALIZED: { 'en-US': 'The current subscription snapshot will be exported as explicit nodes.', 'zh-CN': '当前订阅快照将导出为显式节点。' },
  REMOTE_SOURCE_TARGET_UNSUPPORTED: { 'en-US': 'This target does not support native remote proxy sources.', 'zh-CN': '该目标不支持原生远程代理源。' },
  REMOTE_SOURCE_PROCESSING_UNSUPPORTED: { 'en-US': 'This processing path cannot be represented by the target as a remote source.', 'zh-CN': '该处理路径无法由目标以远程源无损表达。' },
  REMOTE_SOURCE_REQUEST_PROFILE_UNSUPPORTED: { 'en-US': 'The selected request profile cannot be preserved safely by this target.', 'zh-CN': '该目标无法安全保留所选请求兼容模式。' },
  REMOTE_SOURCE_SNAPSHOT_UNAVAILABLE: { 'en-US': 'Refresh and parse this subscription before preserving it remotely.', 'zh-CN': '请先刷新并解析该订阅，再保留远程来源。' },
  REMOTE_SOURCE_MIXED_INPUTS: { 'en-US': 'Combined source lineages require materialized proxy nodes.', 'zh-CN': '合并后的多来源链路需要固化代理节点。' },
  REMOTE_SOURCE_FORCED_BUT_UNSUPPORTED: { 'en-US': 'Remote export was required, but this path has no equivalent native lowering.', 'zh-CN': '当前路径没有等价原生转换，无法满足强制保留远程的要求。' },
  REMOTE_SOURCE_RUNTIME_DRIFT: { 'en-US': 'Target-side refreshes may differ from the current ProxyFlow snapshot.', 'zh-CN': '目标客户端刷新后的节点可能与当前 ProxyFlow 快照不同。' },
  REMOTE_SOURCE_URL_EMBEDDED: { 'en-US': 'The subscription URL will be embedded in the exported configuration.', 'zh-CN': '订阅地址将写入导出的配置。' },
  REMOTE_REQUEST_FALLBACK_NOT_PORTABLE: { 'en-US': 'Only the preferred compatible request identity is portable to the target client.', 'zh-CN': '目标客户端只能使用首选兼容请求标识，无法复现 Runtime 的多标识回退。' },
  ...
  ROUTE_MATCHER_MISSING: { 'en-US': 'This routing rule has no valid matcher.', 'zh-CN': '该分流规则尚未配置有效匹配条件。' },
  ROUTE_TARGET_MISSING: { 'en-US': 'This routing rule has no target.', 'zh-CN': '该分流规则尚未选择目标。' },
  ...
  LOAD_BALANCE_MISSING_SOURCE: { 'en-US': 'This load-balance strategy has no proxy source.', 'zh-CN': '该负载均衡策略尚未连接节点来源。' },
  SELECT_CANDIDATES_EMPTY: { 'en-US': 'This selector has no candidate nodes or strategies.', 'zh-CN': '该手动选择策略尚未添加候选节点或策略。' },
  FALLBACK_CANDIDATES_EMPTY: { 'en-US': 'This fallback strategy has no candidates.', 'zh-CN': '该故障切换策略尚未添加候选节点。' },
  CHAIN_EMPTY: { 'en-US': 'This proxy chain has no hops.', 'zh-CN': '该代理链尚未添加任何跳点。' },
  SUBSCRIPTION_URL_MISSING: { 'en-US': 'This subscription source has no URL.', 'zh-CN': '该订阅来源尚未填写订阅地址。' },
