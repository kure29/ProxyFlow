...
export const LOON_CAPABILITY_MATRIX: readonly LoonCapabilityDecision[] = [
  ...
  { feature: 'Routing baseline', status: 'conditional', reason: 'Pure domain-family and pure IP-family mappings preserve Universal priority; mixed domain/IP routes remain blocked. A local matcher plus an owned Remote Rule is conditional on Universal effective order putting local routes first, matching proven Loon LOCAL_FIRST semantics; Remote-before-Local intent is blocked.', officialReference: `${MANUAL}/rule.md#L5-L11`, diagnostic: 'LOON_ROUTE_ORDER_SEMANTICS_UNSUPPORTED' },
  { feature: 'First-party Service Rules', status: 'conditional', reason: 'Owned Loon rule assets use the evidenced Remote Rule syntax. Local plus first-party Remote Rules is supported only for LOCAL_FIRST-compatible Universal ordering; known Remote-before-Local intent and generalized different-policy Remote-vs-Remote ordering fail closed.', officialReference: LOON_EXAMPLE_CONFIG, diagnostic: 'LOON_REMOTE_RULE_ORDER_SEMANTICS_UNPROVEN' },
