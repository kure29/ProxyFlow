...
export class ServerRuntimeProvider implements SourceFetcher {
  ...
  async health(): Promise<RuntimeHealth> {
    const response = await fetch(`${normalizeBaseUrl(this.config.baseUrl)}/health`, { headers: { Authorization: `Bearer ${this.config.token}` } })
  ...
  async restoreSnapshot(snapshotId: string): Promise<SubscriptionSnapshot> {
    const response = await fetch(`${this.sourceUrl('history/restore')}`, {
      method: 'POST', headers: { Authorization: `Bearer ${this.config.token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ snapshotId }),
    })
  ...
  async saveSchedule(url: string, intervalSeconds: number, enabled = true): Promise<RuntimeSchedule> {
    const response = await fetch(this.sourceUrl('schedule'), {
      method: 'PUT', headers: { Authorization: `Bearer ${this.config.token}`, 'Content-Type': 'application/json' },
  ...
  async clearSchedule() {
    const response = await fetch(this.sourceUrl('schedule'), { method: 'DELETE', headers: { Authorization: `Bearer ${this.config.token}` } })
  ...
  private async request(action: string) {
    const response = await fetch(this.sourceUrl(action), { headers: { Authorization: `Bearer ${this.config.token}` } })
  ...
  private async postAction(action: string) {
    const response = await fetch(`${normalizeBaseUrl(this.config.baseUrl)}/api/v1/projects/${encodeURIComponent(this.source.projectId)}/sources/${encodeURIComponent(this.source.sourceId)}/${action}`, {
      method: 'POST', headers: { Authorization: `Bearer ${this.config.token}` },
    })
...
export function loadRuntimeServiceConfig(): RuntimeServiceConfig | null {
  ...
  try {
    ...
    return parsed && typeof parsed.baseUrl === 'string' && typeof parsed.token === 'string' && parsed.baseUrl && parsed.token ? { baseUrl: parsed.baseUrl, token: parsed.token } : null
  } catch { return null }
}
