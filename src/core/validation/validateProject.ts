...
export function validateGraph(nodes: GraphNode[], edges: GraphEdge[], services = serviceCatalog): ValidationIssue[] {
  ...
  for (const node of nodes) {
    ...
    if (node.data.targetNativeSurgeDnsBehavior !== undefined) {
      if (node.data.blockType !== 'dns' || !isTargetNativeSurgeDnsBehaviorConfig(node.data.targetNativeSurgeDnsBehavior)) {
        add('TARGET_NATIVE_DNS_INVALID', 'Target-native Surge DNS behavior must be valid and attached to a DNS node.', 'error')
      }
    }
  }
  return issues
