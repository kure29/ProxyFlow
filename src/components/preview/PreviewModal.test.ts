...
describe('Preview compatibility summary', () => {
  ...
  it('exposes every supported preview compiler while keeping target identity safe', () => {
    ...
    expect(resolveVisiblePreviewTargets('mihomo', null)).toEqual(['mihomo', 'surge', 'loon', 'shadowrocket'])
    expect(resolveVisiblePreviewTargets('surge', null)).toEqual(['mihomo', 'surge', 'loon', 'shadowrocket'])
    ...
    expect(shouldShowPreviewPausedWarning('shadowrocket')).toBe(false)
  })
})
