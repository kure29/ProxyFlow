...
import { detectSameOriginRuntime, ServerRuntimeProvider } from '../../core/runtime'
...
export function RuntimeServicePanel() {
  ...
  const autoDiscoverySuppressed = useRef(false)

  useEffect(() => {
    if (!config) { setBaseUrl(''); setToken(''); setStatus('idle') }
    else { setBaseUrl(config.sameOrigin ? window.location.origin : config.baseUrl); setToken(config.token); setStatus('connected') }
  }, [config])

  useEffect(() => {
    if (config || autoDiscoverySuppressed.current) return
    let cancelled = false
    void detectSameOriginRuntime().then((detected) => {
      if (!cancelled && detected) setConfig(detected)
    })
  ...
  const disconnectCurrent = () => {
    autoDiscoverySuppressed.current = true
    disconnect()
    close()
  }
  ...
  return <div className="runtime-service-wrap">
    ...
    {open && <div className="runtime-service-popover" role="dialog" aria-label={t('runtime.title')} onClick={(event) => event.stopPropagation()}>
      ...
      <p>{config?.sameOrigin ? t('runtime.sameOriginDescription') : t('runtime.description')}</p>
      {config?.sameOrigin ? <>
        <div className="runtime-service-status"><Check size={13} />{t('runtime.selfHosted')}</div>
        <div className="runtime-service-actions">
          <button className="danger-button secondary-action" onClick={disconnectCurrent}><Unlink size={14} />{t('runtime.disconnect')}</button>
        </div>
        <small>{t('runtime.sameOriginPrivacy')}</small>
      </> : <>
        ...
        <div className="runtime-service-actions">
          ...
          {config && <button className="danger-button secondary-action" onClick={disconnectCurrent}><Unlink size={14} />{t('runtime.disconnect')}</button>}
        </div>
