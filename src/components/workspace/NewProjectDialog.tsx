        ...
        <div className="target-choice-list">
          {PRODUCT_TARGETS.map((item) => {
            ...
            return <button type="button" className={target === item ? 'is-selected' : ''} key={item} onClick={() => setTarget(item)} aria-pressed={target === item}>
              ...
              <span><strong>{definition.label}</strong><small>{t('newProject.baseline', { version: capabilities.baselineVersion })}</small>{item === 'surge' && <small>{t('newProject.targetDescription.surge')}</small>}{item === 'shadowrocket' && <small>{t('newProject.targetDescription.shadowrocket')}</small>}</span>
