...
import { AlertTriangle, Check, Download, Eye, Laptop, LoaderCircle, Network, Server, ShieldCheck, X } from 'lucide-react'
...
import type { MihomoOutputProfile, MihomoRuntimePreset } from '../../types/project'
...
export function WorkspaceExportPanel({ primaryTarget, compiles, onPreview, onSelectTarget }: {
  ...
  return <div className="workspace-export-page">
    ...
    <section className="workspace-export-section" aria-labelledby="export-configuration-title">
      ...
      {activeTarget === 'mihomo'
        ? <div className="export-configuration-grid">
          ...
          <div><span><Laptop size={17} /><strong>{t('workspace.export.tun')}</strong></span><label><span>{t('inspector.mihomoPreset')}</span><select value={mihomoProfile.preset} onChange={(event) => setPreset(event.target.value as MihomoRuntimePreset)}><option value="local-proxy">{t('inspector.mihomoPreset.local')}</option><option value="desktop-tun">{t('inspector.mihomoPreset.tun')}</option></select></label><small>{mihomoProfile.preset === 'desktop-tun' ? t('inspector.mihomoPreset.tunHint') : t('inspector.mihomoPreset.localHint')}</small></div>
          <div><span><Server size={17} /><strong>{t('workspace.export.dns')}</strong></span><strong>{t('workspace.export.resolvers', { count: dnsResolverCount })}</strong><small>{t(mihomoProfile.dnsMode === 'disabled' ? 'inspector.mihomoDnsMode.disabled' : mihomoProfile.dnsMode === 'fake-ip' ? 'inspector.mihomoDnsMode.fake-ip' : 'inspector.mihomoDnsMode.redir-host')}</small></div>
          <details><summary>{t('workspace.export.advanced')}</summary><div><label className="toggle-row compact"><span><strong>{t('inspector.mihomoIpv6')}</strong></span><input type="checkbox" checked={mihomoProfile.ipv6} onChange={(event) => setMihomoProfile({ ipv6: event.target.checked })} /></label><label className="toggle-row compact"><span><strong>{t('inspector.mihomoSniffer')}</strong></span><input type="checkbox" checked={mihomoProfile.sniffer} onChange={(event) => setMihomoProfile({ sniffer: event.target.checked })} /></label><label className="toggle-row compact"><span><strong>{t('inspector.mihomoTcpConcurrent')}</strong></span><input type="checkbox" checked={mihomoProfile.tcpConcurrent} onChange={(event) => setMihomoProfile({ tcpConcurrent: event.target.checked })} /></label></div></details>
        </div>
    ...
    <section className="workspace-export-actions" aria-labelledby="export-actions-title"><div><h2 id="export-actions-title">{t('workspace.export.actionsSection')}</h2><span className={`is-${status.kind}`}>{status.kind === 'ready' ? t('workspace.exportReady') : t('workspace.export.blocked')}</span></div><div><button type="button" className="secondary-action" onClick={() => onPreview(activeTarget)}><Eye size={16} />{t('workspace.export.preview')}</button><button type="button" className="primary-action" disabled={status.kind !== 'ready'} onClick={download}><Download size={16} />{t('workspace.export.download')}</button></div></section>
  </div>
}
