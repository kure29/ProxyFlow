        ...
        <section className="workspace-export-section" aria-labelledby="export-configuration-title">
          ...
          {activeTarget === 'mihomo'
            ? <div className="export-configuration-grid">
              ...
              <div><span><Laptop size={17} /><strong>{t('workspace.export.tun')}</strong></span><label><span>{t('inspector.mihomoPreset')}</span><select value={mihomoProfile.preset} onChange={(event) => setPreset(event.target.value as MihomoRuntimePreset)}><option value="local-proxy">{t('inspector.mihomoPreset.local')}</option><option value="desktop-tun">{t('inspector.mihomoPreset.tun')}</option></select></label>{mihomoProfile.preset === 'desktop-tun' && <><label><span>{t('inspector.mihomoTunStack')}</span><select value={mihomoProfile.tunStack} onChange={(event) => setMihomoProfile({ tunStack: event.target.value as MihomoTunStack })}><option value="mixed">Mixed</option><option value="system">System</option><option value="gvisor">gVisor</option></select></label><label className="toggle-row compact"><span><strong>{t('inspector.mihomoStrictRoute')}</strong><small>{t('inspector.mihomoStrictRouteHint')}</small></span><input type="checkbox" checked={mihomoProfile.strictRoute} onChange={(event) => setMihomoProfile({ strictRoute: event.target.checked })} /></label></>}<small>{mihomoProfile.preset === 'desktop-tun' ? t('inspector.mihomoPreset.tunHint') : t('inspector.mihomoPreset.localHint')}</small></div>
              <div><span><Server size={17} /><strong>{t('workspace.export.dns')}</strong></span><strong>{t('workspace.export.resolvers', { count: dnsResolverCount })}</strong><label><span>{t('inspector.mihomoDnsMode')}</span><select value={mihomoProfile.dnsMode} disabled={mihomoProfile.preset === 'desktop-tun'} onChange={(event) => setMihomoProfile({ dnsMode: event.target.value as MihomoDnsMode })}><option value="disabled">{t('inspector.mihomoDnsMode.disabled')}</option><option value="redir-host">{t('inspector.mihomoDnsMode.redir-host')}</option><option value="fake-ip">{t('inspector.mihomoDnsMode.fake-ip')}</option></select></label><small>{mihomoProfile.preset === 'desktop-tun' ? t('inspector.mihomoTunDnsLocked') : t('inspector.mihomoAdvancedHint')}</small></div>
