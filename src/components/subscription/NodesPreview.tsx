...
import { maskServer, proxyProtocolLabel, REGION_OPTIONS, type SupportedProxyProtocol } from '../../core/proxy'
...
export function NodesPreview({ snapshot, onClose, initialStatus = 'all' }: { snapshot?: SubscriptionSnapshot; onClose: () => void; initialStatus?: 'all' | 'issues' }) {
  ...
  return <div className="nodes-preview-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="nodes-preview" role="dialog" aria-modal="true" aria-labelledby="nodes-preview-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="nodes-preview-table" role="table">
        <div className="nodes-preview-row is-heading" role="row"><span>Name</span><span>Protocol</span><span>Region</span><span>Server</span><span>Port</span><span>Status</span></div>
...
function protocolName(value: string) {
  return ['http', 'socks5', 'shadowsocks', 'trojan', 'vmess', 'vless'].includes(value)
