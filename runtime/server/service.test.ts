...
describe('Runtime Service', () => {
  ...
  it('serves the Web app and authenticates same-origin API requests with an HttpOnly cookie', async () => {
    ...
    const service = createRuntimeService({
      token, sameOrigin: true, staticDirectory: webRoot, version: '1.0.0-rc.6',
    ...
    try {
      ...
      expect(health).toEqual(expect.objectContaining({ version: '1.0.0-rc.6', web: 'ready', backend: 'ready', scheduler: 'ready' }))
