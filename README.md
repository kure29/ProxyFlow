...
当前版本是 ProxyFlow V0.3 Mihomo Compiler MVP。ProxyFlow 已能把 Visual Graph 编译为 Universal IR，再生成真实、可解析的 Mihomo YAML。

这是 Mihomo 功能子集，不代表完整支持 Mihomo。订阅内容不会在编译期下载或解析，节点统计和测速仍是 Mock 数据。
...
Target Compiler Registry 已注册真实 `MihomoCompiler`。sing-box、Surge 等其他 Target 仍未实现。完整说明见 [Core Architecture](docs/architecture.md) 与 [Mihomo Compiler MVP](docs/mihomo-compiler.md)。
...
后端、账号、数据库、真实订阅解析、节点测速、远程规则同步、协议解析、完整 Mihomo Schema、sing-box/Surge Compiler、配置发布 URL 与云同步均未实现。
