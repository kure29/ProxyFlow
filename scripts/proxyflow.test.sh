...
version="${MOCK_HEALTH_VERSION_BEFORE:-1.0.0}"
...
assert_output_not_contains 'ProxyFlow Manager v1.0.0' 'Explicit subcommands do not enter the menu'
...
assert_output 'ProxyFlow Manager v1.0.0' 'TTY no-argument execution opens the manager'
...
assert_output_count_at_least 'ProxyFlow Manager v1.0.0' 2 'An invalid choice returns to the menu loop'
...
assert_output_count_at_least 'ProxyFlow Manager v1.0.0' 2 'Uninstalled actions return to the menu'
...
assert_output '版本：1.0.0' 'The menu reads the running version best-effort'
...
assert_output_count_at_least 'ProxyFlow Manager v1.0.0' 2 'Stopped status returns to the menu'
...
assert_output_count_at_least 'ProxyFlow Manager v1.0.0' 2 'Action failure returns to the menu'
MENU_TEST_DOCKER_INFO_FAIL='0'
...
assert_output_count_at_least 'ProxyFlow Manager v1.0.0' 2 'Logs interruption rerenders the menu'
...
assert_file_contains "${STANDALONE_HOME}/compose.yaml" '    image: ${PROXYFLOW_IMAGE:-ghcr.io/kure29/proxyflow:1.0.0}' 'Embedded Compose uses the immutable 1.0.0 image by default'
